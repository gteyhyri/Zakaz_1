...
def handle_click():
    ...
    try:
        ...
        if not user_id:
            app.logger.error("User ID is missing in request")
            return jsonify({'error': 'User ID is required'}), 400
        
        app.logger.info(f"Processing click for user {user_id}")
        ...
        available_clicks = float(user['available_clicks'])
        if available_clicks < 1:
            app.logger.info(f"No clicks available for user {user_id}")
        ...
        app.logger.info(f"Click processed for user {user_id}. Total: {result['total_clicks']}, Available: {result['available_clicks']}")
        ...
        app.logger.error(f"Error in handle_click: {str(e)}", exc_info=True)
        return jsonify({'error': 'Internal server error'}), 500
    finally:
        if cur:
            cur.close()
