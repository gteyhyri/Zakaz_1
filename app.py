    ...
    try:
        ...
        SET 
            available_clicks = LEAST(
                users.available_clicks + EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - users.last_update)) * 0.1 * users.regen_rate,
                users.max_clicks
            ),
...
def handle_click():
    ...
    try:
        ...
        # Сначала проверяем существование пользователя и создаем при необходимости
        cur.execute("""
        ...
        # Теперь блокируем строку для обновления
        cur.execute("""
